...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        ...
        let y = "yeet" // replace with a string
        let z = 12.6 // replace with anything else!
        x |> should be ofType<int>
        y |> should be ofType<string>

    [<Test>]
    let ``09 A wildcard pattern will match anything`` () =
        let eight = 3 // replace with an integer
        let integ = "9" // replace with a string
        let oof = 'p' // replace with anything else!
        ()
